...
class PlayoffPredictor:
    ...
    def __init__(self, team_stats_path: str, win_prob_model_path: Optional[str] = None, 
        ...
        self.seeds = {}
        self.game_results = {}
        self._injury_cache = {}  # Cache injury data by season to avoid repeated fetches
    ...
    def load_team_stats(self, season: int) -> pd.DataFrame:
        ...
        # Optional: Enhance with historical PBP data
        # This can be enabled by setting use_historical=True when calling load_team_stats
        # For now, we'll check if historical data exists and optionally enhance
        try:
            from src.data.fetching.load_historical_data import enhance_team_stats_with_pbp
            # Only enhance if we have basic stats but want more detail
            if not df.empty and season >= 1999:
                enhanced = enhance_team_stats_with_pbp(df, season)
                if not enhanced.empty and len(enhanced) == len(df):
                    # Only use if enhancement was successful
                    df = enhanced
                    logger.debug(f"Enhanced team stats with historical PBP data for {season}")
        except ImportError:
            # Historical data module not available, skip
            pass
        except Exception as e:
            logger.debug(f"Could not enhance with historical data: {e}")
    ...
    def _load_win_prob_model(self, team_stats_df: pd.DataFrame, use_advanced_features: bool = True):
        """
        Load or create improved win probability model using XGBoost.
        
        Args:
            team_stats_df: Team statistics DataFrame
            use_advanced_features: Whether to add advanced features (recent form, SOS, etc.)
        ...
        try:
            ...
            if use_advanced_features:
                try:
                    ...
                    try:
                        ...
                        if season:
                            game_results_df = self.load_game_results(season)
                    except:
                        pass
                    ...
                    logger.info("Added advanced features to model")
                except Exception as e:
            ...
            # Train improved model with better default hyperparameters
            if USE_XGBOOST:
                clf = make_pipeline(
                    ...
                    XGBClassifier(
                        ...
                        learning_rate=0.1,
                        subsample=0.8,
                        colsample_bytree=0.9,  # Added for better generalization
                        random_state=42,
                        n_jobs=-1,
                        eval_metric='logloss'
                    )
            ...
            try:
                ...
                calibrated_clf.fit(pairs_X, pairs_y)
                logger.info(f"Trained and calibrated win probability model with {len(features)} features")
                return calibrated_clf, features
            except ImportError:
                logger.debug("Calibration not available, using uncalibrated model")
    ...
    def predict_matchup_win_prob(self, team1: str, team2: str, 
        ...
        if win_prob_model is not None and features is not None:
            try:
                ...
                # This is critical because StandardScaler expects the exact same number of features
                team1_feats = []
                team2_feats = []
                
                for feat in features:
                    # Get feature value if available, otherwise use 0
                    val1 = team1_stats.get(feat, 0)
                    val2 = team2_stats.get(feat, 0)
                    ...
                    team1_feats.append(float(val1))
                    team2_feats.append(float(val2))
                
                # Convert to numpy arrays
                team1_feats = np.array(team1_feats)
                team2_feats = np.array(team2_feats)
                
                # Calculate difference (team1 - team2)
                diff = (team1_feats - team2_feats).reshape(1, -1)
                
                # Predict probability - model expects exact feature count
        ...
        if base_prob is None:
            ...
            # Use less aggressive sigmoid to avoid extreme values
            base_prob = 1 / (1 + np.exp(-3 * wp_diff))  # Reduced from 5 to 3 for less extreme predictions
        ...
        try:
            ...
            # Get season and week from team_stats_df if available
            season = team_stats_df.get('season', pd.Series()).iloc[0] if 'season' in team_stats_df.columns else None
            week = team_stats_df.get('week', pd.Series()).iloc[0] if 'week' in team_stats_df.columns else None
            
            if season:
                # Cache injury data by season to avoid repeated fetches
                cache_key = f"{season}_{week if week else 'all'}"
                if cache_key not in self._injury_cache:
                    injury_df = fetch_injury_reports(season, week)
                    self._injury_cache[cache_key] = injury_df
                else:
                    injury_df = self._injury_cache[cache_key]
                
                if injury_df is not None and not injury_df.empty:
                    # Calculate injury impact for both teams
                    team1_impact = calculate_team_injury_impact(injury_df, team1, week)
                    team2_impact = calculate_team_injury_impact(injury_df, team2, week)
                    
                    # Convert injury scores to adjustment factors
                    max_score = 5.0
                    team1_injury_adj = -0.20 * min(team1_impact['injury_score'] / max_score, 1.0)
                    team2_injury_adj = -0.20 * min(team2_impact['injury_score'] / max_score, 1.0)
                    
                    # QB injury gets extra penalty
                    if team1_impact['key_players_out'] > 0:
                        team1_injuries = injury_df[injury_df['team'] == team1]
                        if week and 'week' in team1_injuries.columns:
                            team1_injuries = team1_injuries[team1_injuries['week'] == week]
                        qb_out = team1_injuries[
                            (team1_injuries['position'].str.upper() == 'QB') &
                            (team1_injuries['report_status'].isin(['Out', 'Doubtful', 'IR']))
                        ]
                        if not qb_out.empty:
                            team1_injury_adj -= 0.12
                    
                    if team2_impact['key_players_out'] > 0:
                        team2_injuries = injury_df[injury_df['team'] == team2]
                        if week and 'week' in team2_injuries.columns:
                            team2_injuries = team2_injuries[team2_injuries['week'] == week]
                        qb_out = team2_injuries[
                            (team2_injuries['position'].str.upper() == 'QB') &
                            (team2_injuries['report_status'].isin(['Out', 'Doubtful', 'IR']))
                        ]
                        if not qb_out.empty:
                            team2_injury_adj -= 0.12
                    
                    # Apply injury adjustments (team1's injuries help team2, and vice versa)
                    adjusted_prob += team1_injury_adj
                    adjusted_prob -= team2_injury_adj
                    
                    if team1_injury_adj < 0 or team2_injury_adj < 0:
                        logger.debug(f"Injury adjustments: {team1}={team1_injury_adj:.1%}, {team2}={team2_injury_adj:.1%}")
        except ImportError:
            logger.debug("Injury tracking not available")
        except Exception as e:
            logger.debug(f"Could not apply injury adjustments: {e}")
        
        # Use less aggressive clipping to allow more nuanced probabilities
        # This allows probabilities between 0.10 and 0.90, giving more granularity
        return np.clip(adjusted_prob, 0.10, 0.90)
    ...
    def simulate_full_playoffs(self, season: int, n_simulations: int = 1000) -> Dict:
        """
        ...
        # Add advanced features (recent form, SOS, etc.) if game results available
        try:
            from src.model_improvements import add_advanced_features
            game_results_df = self.load_game_results(season)
            if not game_results_df.empty:
                df = add_advanced_features(df, game_results_df, season=season)
                logger.info("Added advanced features for playoff simulation")
        except Exception as e:
            logger.debug(f"Could not add advanced features: {e}")
        
        # Load improved win probability model
        win_prob_model, features = self._load_win_prob_model(df, use_advanced_features=True)
    ...
    def get_frozen_brackets(self, simulation_output: Dict, conference: str = None):
        """
        Get FrozenBracket models from simulation output.
        
        Uses the new adapter to convert simulation output to canonical models.
        ...
        Returns:
            FrozenBracket for specified conference, or dict with both if conference is None
        """
        from src.adapters.simulation_adapter import SimulationOutputAdapter
        adapter = SimulationOutputAdapter()
        
        if conference:
            return adapter.to_frozen_bracket(simulation_output, conference)
        else:
            return {
                'AFC': adapter.to_frozen_bracket(simulation_output, 'AFC'),
                'NFC': adapter.to_frozen_bracket(simulation_output, 'NFC')
            }
...
def main():
    ...
    parser.add_argument('--output', type=str, default='output/playoffs/playoff_predictions.json',
